...
class InvokerBytecodeGenerator {
    ...
    private void maybeDump(final byte[] classFile) {
        if (DUMP_CLASS_FILES) {
            maybeDump(lambdaForm.kind, className, classFile);
        }
    }

    static void maybeDump(final String className, final byte[] classFile) {
        maybeDump(null, className, classFile);
    }

    // Also used from BoundMethodHandle
    @SuppressWarnings("removal")
    static void maybeDump(Kind kind, final String className, final byte[] classFile) {
        if (DUMP_CLASS_FILES) {
            ...
            new java.security.PrivilegedAction<>() {
                public Void run() {
                    try {
                        ...
                        if (kind != null) {
                            System.out.println("dump: " + kind + " " + dumpFile.getAbsolutePath());
                        } else {
    ...
    private static MemberName lookupPregenerated(LambdaForm form, MethodType invokerType) {
        ...
        switch (form.kind) {
            ...
            case FILTER_ARG: {
                if (USE_NEW_CODE) {
                    name = form.lambdaName;
                    return resolveFrom(name, invokerType, LambdaFormEditor.Holder.class);
                } else {
                    return null;
                }
            }
    ...
    static MemberName generateCustomizedCode(LambdaForm form, MethodType invokerType) {
        ...
        if (pregenerated != null) {
            return pregenerated; // pre-generated bytecode
        }
