...
RawFrameStream::RawFrameStream(JavaThread* thread, jint mode, RegisterMap* rm, Handle cont_scope, Handle cont)
  ...
  if (cont.is_null()) {
    ...
    _frame  = Continuation::last_frame(cont(), rm);
    _cont_entry = nullptr;
  }
...
void RawFrameStream::next() {
  ...
  do {
    ...
    if (_decode_offset != 0) {
      ...
      oop cont = this->cont();
      if (cont != (oop)nullptr && Continuation::is_continuation_entry_frame(_frame, _map)) {
        oop scope = jdk_internal_vm_Continuation::scope(cont);
        if (_cont_scope.not_null() && scope == _cont_scope()) {
          break;
        }
      ...
      assert(!Continuation::is_scope_bottom(_cont_scope(), _frame, _map), "");
